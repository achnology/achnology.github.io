...
const websiteURL = 'http://127.0.0.1:5500/people_search.html';
...
test('same external css for all html pages', async ({ page }) => {
   ...
   const cssFile = await page.locator('link[rel="stylesheet"]').getAttribute('href')

   await page.getByRole('link', { name: 'Vehicle search' }).click();
   await expect(page.locator('link[rel="stylesheet"]')).toHaveAttribute('href', cssFile);
